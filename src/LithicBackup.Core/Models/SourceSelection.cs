...
{
    // WITHDRAWN: a model-level tristate "repair" that promoted a partial
    // directory to fully-selected when every child it *listed* was selected.
    //
    // It shipped in 1.3.1 and had to be pulled the same day. The saved model
    // holds only the nodes the user has touched, so a partial parent's listed
    // children are NOT the complete set: C:\ was stored partial with 6 listed
    // children, all selected, while the volume has 64 subdirectories. Promoting
    // it to selected silently added all 64 to a backup set.
    //
    // The reasoning that justified it - "an unlisted child inherits its parent,
    // so every-listed-child-selected cannot conceal an exclusion" - is exactly
    // backwards under a partial parent, where unlisted children are excluded by
    // definition. Reconciling a tristate UPWARD from a partial list can only
    // ever over-select, and over-selecting a source is a data-volume and
    // privacy problem, not a cosmetic one.
    //
    // Anything attempted here again must only ever narrow a selection
    // (selected -> partial when a listed child is excluded), never widen it,
    // and must be tested against a node whose children are a strict subset of
    // what is on disk.
