...
- **Continuous** — track file changes and back up exactly the files that changed, once each file goes quiet. On NTFS volumes this uses the USN change journal, which records every change the moment it happens — including those that occur while the Worker is offline — so nothing is missed across reboots or service restarts, and only changed files are versioned rather than rescanning whole drives. Push notifications from the journal wake the Worker the instant a watched volume changes, so a change is picked up immediately instead of at the next poll. Each file is debounced individually (with a cap so a constantly-written file can't starve its own backup), which prevents thrashing while you're actively editing. The timing is governed by two **global** rule sets, edited once under **Settings ▸ Continuous** and shared by every continuous set:
  - **Settle time (by file size)** — how long a file must sit unchanged before it's backed up, tiered by size so a brief pause mid-write doesn't trigger a wasteful copy of a huge file. Defaults: files under 1 MB settle in 0.5 s, up to 100 MB in 3 s, up to 1 GB in 10 s, larger in 30 s. Because a file that's actively being written never goes quiet, the settle time is self-adjusting — it naturally holds off until a download or long save actually finishes.
  - **Maximum wait (by name / path mask)** — some files change constantly and never settle (busy logs, append-only session files), so the settle time alone would never fire. For files matching a mask tier, the Worker versions them anyway after the tier's cap, even mid-write — safe because their partial contents are a meaningful prefix. By default this opts in `*.log`, `*.jsonl`, `*.ndjson`, `*.json`, `*.out`, and `*.trace` at a 5-minute cap. Files matching **no** tier have an infinite max wait: they're only ever copied once they settle, which is the safe choice for downloads and databases whose mid-write contents would be torn or corrupt.
  - **Backstop polling** — the Worker also re-checks the change journal on a fixed timer (~30 s) as a safety net behind the push notifications, so nothing is missed even if a notification is ever dropped. This runs in a single shared loop and needs no configuration — with push notifications doing the real work, the old per-set poll-interval setting was removed.
...
**Continuous backup keeps running during a full backup.** Some events — an
unusually long outage, or a source volume's change journal filling up while the
Worker was busy — make Lithic fall back to a full reconciling scan, which on a
large set can take hours. That scan no longer stops continuous backup for its
duration: at each of its periodic save points it picks up whatever has changed
since and backs those files up alongside it, so work you do while it runs is
protected straight away instead of waiting for it to finish. It also keeps
reading the change journal throughout, which matters because a journal left
unread for hours can overflow and lose its history — which is what forced the
full scan in the first place.

To stop that happening in the first place, the Worker **enlarges the Windows
change journal** on each source volume it watches (to 512 MB, from the 32 MB
Windows default) the first time it opens one. It only ever enlarges — never
shrinks — because that journal is shared with Windows Search and other tools.
This needs the Worker service, which runs with the necessary privileges; if it
can't, continuous backup still works and simply falls back to a full scan more
often. Separately, continuous sets now run one full scan a day as a safety net,
which is what catches individually deleted files and anything a previous run
couldn't read.

**Full-destination warning** — because the Worker runs in the background with no window, a continuous set whose destination drive fills up would otherwise fail silently: new versions simply stop being written with nothing to tell you. While the app is running, it watches the destination drive of every set that has continuous backup enabled and pops up a warning dialog when one is (practically) full, so you can free up space before you miss any versions. The dialog stays on screen until you dismiss it (rather than a tray balloon that self-dismisses after a few seconds), so you'll still catch it if you were away from your desk. It appears once per fill-up (not repeatedly) and only warns again for that drive after it recovers and then fills up once more — a different destination filling up is warned separately. The warning is **debounced**: the drive has to read low on three checks in a row (about a minute and a half) before you're interrupted, because free space can crater for a moment and then recover entirely on its own — most often when Windows' Volume Shadow Copy Service grows a snapshot, exhausts the volume, and Windows responds by discarding the snapshot. The dialog also **names the time** the drive ran out of space rather than claiming it's full right now, and is **dropped if the drive recovers** before the dialog actually reaches the screen — so a warning you read an hour later can't contradict what Explorer is showing you. Separately, **every** backup set (not just continuous ones) shows a persistent "destination drive full" line on its row in the main list whenever its destination is full; this status is re-checked every 30 seconds while the app is open and clears itself the moment space is freed, so you always see the current state at a glance.
...
- Checking a folder includes everything in it — including sub-folders you had unchecked before, whether or not they are expanded. Unchecking it takes everything in it out
...
- A few things in a drive's root are never user data and are never backed up, even when the whole drive is selected: the recycle bin (`$Recycle.Bin` — what you've deleted), `System Volume Information`, and Windows' paging, hibernation and swap files. They don't appear in the tree. Copies of them made by older versions stay in the backup until you release them with Cleanup, where they show as excluded files
...
- Turning "Auto-include new" **off** on a checked folder keeps exactly what is in it now. A folder created in it afterwards shows **unchecked**, and the folder itself partly checked, because that is what the backup does — tick the new folder, or the parent, to include it
...
- **Cleanup** — a single unified view that surfaces files and directories worth removing across the catalog and the backup destination. Categories include directories removed from sources, files and directories deleted from disk (a file counts as deleted when it is gone even if its folder is still there — so moving a folder's contents elsewhere and leaving the folder behind is detected), files matching configured or manual exclusion patterns, excess versions beyond the retention tiers (limited to actual `_prev` records on disk, so the category stays empty when no real history exists yet), and catalog duplicates (stray rows pointing at the current copy of a file — typically left over from re-running "Seed from existing backup" on the same destination). The results are resizable three ways, which matters because the tree indents with every level and deep paths otherwise run out of room: drag the dividers in the column header to narrow **Files** and **Size** (the Directory column takes whatever they give up), drag the divider between two category panels to make one wider (double-click it to reset), or set how many category panels sit per row (1-3). All three are remembered between sessions. An optional "Scan destination filesystem" step adds two more categories — untracked files (present on disk but not in the catalog) and catalog-deleted files (marked deleted but still physically present). The "Clean Selected" action both purges the matching catalog records and physically removes the corresponding files from the destination (when one is configured), then sweeps any newly-empty directories. A **Cancel** button appears while a cleanup is running. What cancelling costs depends on how far it has got, and it tells you which: during the catalog update nothing is changed at all (that half runs in a single transaction and rolls back in full, and no backed-up files have been touched yet), while once it has moved on to deleting files the ones already deleted stay deleted and the rest keep their catalog entries, reappearing under "catalog-deleted files" the next time you scan the destination. If the destination is configured but not currently connected, Cleanup says so in a banner, disables the destination scan and reconcile, and refuses to clean at all rather than doing the catalog half on its own — which would free no space and would drop those files out of the catalog scan. Catalog-duplicate cleanups remove only the extra rows; the physical file is left alone. Source volumes that aren't currently connected are skipped and named in the summary rather than being reported as deleted, since an unplugged drive answers "missing" for everything on it. The same view also offers a **Reconcile Catalog with Destination** tool that repairs catalog rows that have drifted out of sync with what's actually on disk: it flips stale file-reference rows back to plain when their content has been materialised into a real file (hash-verified before any change), and prunes active rows whose backing content is missing from the destination. Reconcile is dry-run first — **Analyze** reports exactly what it would change and nothing is modified until you click **Apply** — and it never prunes when the destination is disconnected or empty, so an unmounted drive can't be mistaken for a mass deletion.
...
- **Edit backup sets** — modify sources, options, name, and schedule of existing sets. Nothing you change is saved until you click **Save**, or answer **Yes** when you close the window with unsaved changes. **Cancel** throws the whole edit away, asking first if you changed anything, and closing without changes saves nothing, so a stray click can't reach your backups. **Seed from Existing Backup** is the one exception: it records files as backed up immediately, so with unsaved changes it asks to save them first. **Largest Files** opened from the editor is part of the same edit: what you untick there appears in the editor's exclusion list and is saved or discarded with everything else. Opened on its own, Largest Files has its own **Save** and asks before closing with unsaved changes. A selected folder that no longer exists (deleted or moved) stays selected, so it's backed up again if a folder reappears at that path. The editor shows it greyed with a "(not found)" label so you can see it and untick it. A bar above the tree says how many such entries the selection holds, including ones inside folders you haven't expanded, and **Forget them** removes them all at once; after that, a folder re-created at one of those paths counts as new. A source drive or share that isn't connected when you open the editor is shown as "(not connected)" and kept — Forget leaves it alone too. When you *save* a change to a set's source selection, Lithic Backup reconciles the destination with the new sources instead of waiting for the next Cleanup: if you dropped folders, it shows a read-only review tree of the backed-up files that are no longer covered and offers to remove their destination copies immediately — an all-or-nothing purge that also marks the catalog rows deleted and repairs any references left stale by the deletions (the same reconcile the Cleanup view runs). If the destination isn't connected the removal is declined outright and the files are left for a later Cleanup, for the same reason. If you added folders, it shows a matching preview tree of the files newly covered and offers to back up **only those files**, immediately and without re-scanning the rest of the set — it already walked them, so there is nothing to rediscover (a full scan of a large set takes minutes to hours). That backup shows its progress in the set's row like any other: percentage, bytes copied, time remaining, the file being copied, and Pause and Cancel. Anything else that changed is left to continuous backup or the next scheduled run, and the prompt says so; the ordinary **Backup** button still scans everything. The removal prompt is equally explicit that it deletes only the listed backed-up copies and never touches your source files. if those files turn out to be already covered by the set's existing selection (because a parent folder auto-includes new subfolders) it says so and offers to run the backup anyway, rather than appearing to do nothing. The scanning step no longer blocks the rest of the app — it only reads, so you can carry on working in other windows while it runs. Both review trees group files by location (deeper folders collapsed) and are click-sortable by name, file count, or size (shown in bytes, as elsewhere); neither has per-item checkboxes since each action is all-or-nothing. The two offers are independent, so both can appear after an edit that both added and removed sources; declining either just leaves that work for Cleanup or the next backup. Nothing is prompted when you close the dialog without saving. The modify dialog also includes a **Clear Backup History** action that wipes the catalog's record of what's been backed up (disc and file entries) while keeping all settings — the next backup then re-copies everything. Files already written to the destination are not deleted.
...
- **Concurrent backups** — each backup set runs independently in the GUI, with its own progress display, pause/abort controls, and completion summary shown inline on the set's row. The progress display always names the file actually being worked on. When duplicate detection reads a large file in full to see whether it is already backed up, the display says "Checking for duplicates…" with that file's own progress bar, rather than appearing stuck. Starting a backup on one set never freezes the window or disables the actions on the others, so multiple sets can scan, copy, and verify at the same time.
...
- **Every task opens in its own window** — Cleanup, Restore, Verify Integrity, Backup Coverage, Test Disc, Largest Files, Find File and Catalog-free Restore each open as an independent window with its own taskbar button, rather than replacing the main screen. Several can be open at once, on different sets or the same one, and they stay put: starting or finishing a backup no longer navigates away and discards what you were doing (a long Cleanup destination scan used to be lost that way). They can sit behind the main window and survive it being minimised to the tray. Opening the *same* task again for a set just focuses the window that is already open, and opening a task that could interfere with other work on that set — another window that changes the same catalog or deletes the same backed-up files, or a backup currently running for it — warns first and names the conflict, leaving the choice to you.
...
- **Cache maintenance** — Lithic Backup keeps two on-disk caches (directory sizes and file hashes) so the source tree can show sizes instantly and a backup doesn't re-read files it has already hashed. They only ever grow: an entry is written for every directory walked and isn't removed when that directory is deleted, so on a long-lived install a large share of the entries can point at paths that no longer exist. **File → Settings → Caches** reports each cache's size on disk and entry count, and offers two actions. **Compact now** drops entries whose file or folder is gone, rebuilds the tables in a more compact layout (the original schema stored every path twice, so this alone typically saves about a third), and returns the freed space to the filesystem — reporting how much was freed. On the author's own 517 MB size cache this dropped 388,745 dead entries (26%) and brought the file down to 234 MB. Finding out which paths are gone means asking the disk about every one of them, which is slow on a large cache — about a quarter of an hour for a million and a half entries — so it runs in the background with live progress, you can keep working, and stopping it keeps everything it had finished. Entries on a drive that isn't currently connected are deliberately left alone rather than assumed deleted, and you're told which drives were skipped. **Clear caches** discards everything after a confirmation. Neither can lose data — anything discarded is recomputed on demand, at the cost of re-hashing files and recomputing directory sizes the next time they're needed.
