...
- **Drive-letter resilience** — each directory-backup set remembers its destination by the drive's stable Windows volume identity (volume GUID), not just its drive letter. If Windows reassigns the drive a different letter (common with external/USB drives), LithicBackup detects this when the set next runs, follows the volume to its new letter automatically, updates the stored path, and notes the change (a status message in the app, a log entry in the background service). You keep picking and seeing a familiar drive letter; the durable identity underneath does the work. Sets created before this feature backfill their volume identity the first time the drive is connected and resolvable.
