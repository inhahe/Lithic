...
- **Continuous** — track file changes and back up exactly the files that changed, once each file goes quiet. On NTFS volumes this uses the USN change journal, which records every change the moment it happens — including those that occur while the Worker is offline — so nothing is missed across reboots or service restarts, and only changed files are versioned rather than rescanning whole drives. Push notifications from the journal wake the Worker the instant a watched volume changes, so a change is picked up immediately instead of at the next poll. Each file is debounced individually (with a cap so a constantly-written file can't starve its own backup), which prevents thrashing while you're actively editing. The timing is governed by two **global** rule sets (edited once under **Settings ▸ Continuous**, shared by every continuous set), plus a per-set poll interval:
  ...
  - **Poll interval** — how often the Worker re-checks the journal as a backstop to the push notifications (default 30 s, minimum 5, set per set). The Worker runs a single shared poll loop, so the smallest interval across all your continuous sets sets the pace.
