...
- **Continuous** — track file changes and back up exactly the files that changed, after a configurable quiet period (default: 60 seconds of no changes). On NTFS volumes this uses the USN change journal, which records every change the moment it happens — including those that occur while the Worker is offline — so nothing is missed across reboots or service restarts, and only changed files are versioned rather than rescanning whole drives. Each file is debounced individually (with a 5-minute cap so a constantly-written file can't starve its own backup), which prevents thrashing while you're actively editing. Two knobs tune the responsiveness: the **quiet period** (how long a file must sit unchanged before it's backed up) and the **poll interval** (how often the Worker checks for changes, default 30 seconds, minimum 5). Lowering both makes new versions appear sooner. The Worker runs a single shared poll loop, so the smallest poll interval across all your continuous sets sets the pace.
