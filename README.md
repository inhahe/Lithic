...
For the worst case — when you can't run LithicBackup at all — a standalone Python script, `tools/lithic_restore.py`, performs the same catalog-free restore using only the Python standard library (no dependencies, Python 3.8+). Drop it on any machine with Python and point it at a backup folder:
